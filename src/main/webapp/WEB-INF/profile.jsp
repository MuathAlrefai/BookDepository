...
<%-- CONTENT --%>
<br><br>
    <div style="margin-left: 50px; margin-right: 50px; display: flex; justify-content: space-evenly; align-items: center;">

      <c:if test="${currentUser.id == 1}">
        <div>
          <div>
            <h3>Admin Management</h3><hr>
            <a href="/usersList" class="btn btn-outline-primary">Customers Info</a>
            <a href="/bookList" class="btn btn-outline-primary">Books Info</a>
          </div>
          <br> <br>
          <div style="border: 1px solid black; border-radius: 5px;  padding: 10px;">
            <h3>Site Stats</h3>
            <p>Registered Users: <c:out value="${users.size()}"/></p>
            <p>Books: <c:out value="${books.size()}"/></p>
            <p>Authors: <c:out value="${authors.size()}"/></p>
            <p>Publishers: <c:out value="${publishers.size()}"/></p>
            <p>Date: <c:out value="${date}"/></p>
            <p>Time: <c:out value="${time}"/></p>
          </div>
        </div>
      </c:if>
      <div class="text-center" style="border: 1px solid black; border-radius: 5px;">
        <div class="title">
          <h3>Account Information</h3><hr>
        </div>
        <img src="${currentUser.avatar}" alt="avatar" style="width: 50%; border-radius: 50%;"> <br> <br>
        <p>Name: <c:out value="${currentUser.userName}"/></p>
        <p>Email: <c:out value="${currentUser.email}"/></p>
        <p>Member Since: <fmt:formatDate value="${currentUser.createdAt}" type="date" pattern="dd-MMM-yyyy"/></p>

        <a href="/followingList" class="btn btn-outline-primary">Authors You Follow</a>
        <a href="/purchaseList" class="btn btn-outline-primary">Purchase History</a>
        <hr>
        <h3><a href="/editProfile" class="btn btn-outline-dark">Edit profile</a> | <a href="/editPassword" class="btn btn-outline-dark">Change password</a></h3>
      </div>
      <img src="/assets/profilePic.png" alt="profilePic" style="width: 20%;">
    </div>
