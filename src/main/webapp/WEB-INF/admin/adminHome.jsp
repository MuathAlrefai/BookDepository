...
<!-- NAV BAR -->
<nav class="p-4 navbar navbar-expand-lg navbar-light align-content-center" style="background-color: rgb(73, 54, 72); box-shadow: -1px 4px 5px black;">
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse flex-box" id="navbarSupportedContent">
            ...
            <form class="d-flex" role="search" style="margin-right: 25%; width: 50%;">
                <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search Books" style="border-radius: 5px;">
                <button class="btn btn-info" type="submit" style="color: white;" hidden>Search</button>
            </form>
            <ul>
                <c:if test="${currentUser != null}">
                    <li class="nav-item">
                        <a class="btn btn" style="background-color: cornflowerblue; color: white;" href="/logout">Logout</a>
                    </li>
                </c:if>
                <c:if test="${currentUser == null}">
...
<div class="slider" x-data="{start: true, end: false}">
    <div class="slider__content" x-ref="slider" x-on:scroll.debounce="$refs.slider.scrollLeft == 0 ? start = true : start = false; Math.abs(($refs.slider.scrollWidth - $refs.slider.offsetWidth) - $refs.slider.scrollLeft) < 5 ? end = true : end = false;">
        <c:forEach var="book" items="${specificBook}">
